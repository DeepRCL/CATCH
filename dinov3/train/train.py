...
def do_train(cfg, model, resume=False):
    ...
    # watchdog_handles = install_nan_watchdog(model.student["backbone"], name_prefix="student.backbone")
    # logger.info("NaN watchdog hooks attached to student backbone.")
    if distributed.is_main_process():
        wandb.init(project="dinov3", name="dinov3_vit7b16_pretrain")
        # wandb.watch(model.student["backbone"], log="all", log_freq=1)
    ...
    for data in metric_logger.log_every(
        ...
        if cfg.optim.clip_grad:
            for k, v in student.items():
                ...
                if grad_norm.item() > 300:
                    logger.warning(f"Grad norm for {k} is {grad_norm} on rank {distributed.get_rank()}")
...
def main(argv=None):
    ...
    with torch.device("meta"):
        model: SSLMetaArch = meta_arch(cfg)
