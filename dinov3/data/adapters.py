...
class DatasetWithEnumeratedTargets(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Tuple[Any, int]]:
        batch_data = self._dataset[index % self._size]
        image, target, label_type_list, points_coords, dx, dy, pred_view = batch_data["image"], batch_data["target"], batch_data["label_type_list"], batch_data["points"], batch_data["dx"], batch_data["dy"], batch_data["pred_view"]
        if index >= self._size:
            assert self._pad_dataset
            return image, (-1, target), label_type_list, points_coords, dx, dy, pred_view
        target = index if target is None else target
        # return image, (index, target), label_type_list, points_coords, dx, dy, pred_view
        return {
            "image": image,
            "target": (index, target),
            # "sample_index": sample_index,   # replaces (index, target) tuple
