...
def log_overlay_masks(image, dense_pred,  gt, title=None):
    """
    ...
    # Make alpha masks
    dense_gt_label = gt
    # gt[:4].argmax(axis=0).detach().cpu().numpy()
    print(f" in fig ovelay dense_gt_label:{dense_gt_label.shape},dense_pred :{dense_pred.shape} ")
    ...
    print(f"dense_gt_label:{np.unique(dense_gt_label)}")
    print(f"dense_pred:{np.unique(dense_pred)}")
    ...
    # # ===== Middle: IMAGE + Dense_PRED =====
    # axes[2].imshow(unlabeled_image, cmap="gray", vmin=0, vmax=1)
    # # axes[2].imshow(dense_pred, cmap="viridis", vmin=0, vmax=4, alpha=alpha_dense_pred)
    # axes[2].set_title("Other View Image")
    # axes[2].axis("off") 
...
def train_segmentation(
    ...
    print(f"global_best_metric_values: {global_best_metric_values}")
    # 5- train the model
        ...
        if total_iter % config.eval.eval_interval:
            ...
            if is_better:
                ...
                torch.save(
                    ...
                    os.path.join(config.output_dir, f"model_{total_iter}.pth"),
                )
        # break    
