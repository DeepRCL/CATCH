...
class MaskToTensor(torch.nn.Module):
    ...
    def forward(self, img, label):
        if isinstance(label, np.ndarray):
            return v2.ToTensor()(img), v2.ToTensor()(label)
        return img, Mask(label)


class ToTensor(torch.nn.Module):
    def forward(self, img, label):
        return torch.from_numpy(img).unsqueeze(0), torch.from_numpy(label).unsqueeze(0)
...
def make_segmentation_eval_transforms(
    *,
    ...
    transforms_list.append(NormalizeImage())
