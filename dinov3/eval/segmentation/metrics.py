...
SEGMENTATION_METRICS = ["mIoU", "acc", "aAcc", "dice","lowest_dice", "fscore", "precision", "recall"]
...
def calculate_segmentation_metrics(
    ...
    return {
        ...
        "lowest_dice": metrics_dict["lowest_dice"].nanmin(),
...
def preprocess_nonzero_labels(label, ignore_index=0):
...
def calculate_intersect_and_union(pred_label, label, num_classes, ignore_index=0, reduce_zero_label=False):
...
def total_area_to_metrics(
    ...
    if isinstance(metrics, str):
        metrics = [metrics]
    allowed_metrics = ["mIoU", "dice","lowest_dice", "fscore"]
